...
% Level 1
\node[current] (o1) at (0,2) {$o_1$};
\node[rejected] (o2) at (2,2) {$o_2$};
\node[queued] (o3) at (4,2) {$o_3$};
\node[rejected] (o4) at (6,2) {$o_4$};

% Level 2
\node[candidate] (o5) at (0,0) {$o_5$};
\node[candidate] (o6) at (2,0) {$o_6$};
...
% Candidate edges from o1
\draw[candidedge] (o1) -- (o5);
\draw[candidedge] (o1) -- (o6);
...
\node[draw=none, rectangle, anchor=north west, align=left, font=\scriptsize, text=black]
    at (-0.5,-0.8) {$\mathcal{O}_{\text{sub}} = \{o_5, o_6\}$, \quad $Q = \{(1, o_3, [o_0,o_3])\}$};
\end{tikzpicture}
